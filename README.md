...
1. **Blackout window** — if active, the tail message is suppressed
2. **Minimum interval** — if not enough time has passed since the last play, skip
3. **SkywarnPlus WX alert** — if the `wx-tail.wav` file is larger than `SilenceThreshold` bytes, it plays the WX file (SkywarnPlus writes a small silent placeholder when no alerts are active)
